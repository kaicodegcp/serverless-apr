...
func getTerraformDir(t *testing.T) string {
	...
	return filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main", "wrappers", "schema"))
}

func getVarFile(t *testing.T) string {
	tfDir := getTerraformDir(t)
	return filepath.Join(tfDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-schema.auto.tfvars")
}
...
func TestSchema_InitValidate(t *testing.T) {
	...
	t.Log("Init + Validate passed for schema")
}
...
func TestSchema_ApplyDestroy(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	full_name := terraform.Output(t, opts, "full_name")
	require.NotEmpty(t, full_name, "full_name should not be empty")

	t.Logf("id = %s", id)
	t.Logf("name = %s", name)
	t.Logf("full_name = %s", full_name)
}
...
func TestSchema_VariableValidation(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Variable validation passed for schema")
}
...
func TestSchema_InputAssertions(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Input assertions passed for schema")
}
