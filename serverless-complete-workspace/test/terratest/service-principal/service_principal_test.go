...
func getTerraformDir(t *testing.T) string {
	...
	return filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main", "wrappers", "service-principal"))
}

func getVarFile(t *testing.T) string {
	tfDir := getTerraformDir(t)
	return filepath.Join(tfDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-service-principal.auto.tfvars")
}
...
func TestServicePrincipal_InitValidate(t *testing.T) {
	...
	t.Log("Init + Validate passed for service-principal")
}
...
func TestServicePrincipal_Plan(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Plan passed for service-principal")
}
...
func TestServicePrincipal_ApplyDestroy(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	application_id := terraform.Output(t, opts, "application_id")
	require.NotEmpty(t, application_id, "application_id should not be empty")
	display_name := terraform.Output(t, opts, "display_name")
	require.NotEmpty(t, display_name, "display_name should not be empty")
	acl_principal_id := terraform.Output(t, opts, "acl_principal_id")
	require.NotEmpty(t, acl_principal_id, "acl_principal_id should not be empty")

	t.Logf("id = %s", id)
	t.Logf("application_id = %s", application_id)
	t.Logf("display_name = %s", display_name)
	t.Logf("acl_principal_id = %s", acl_principal_id)
}
...
func TestServicePrincipal_VariableValidation(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Variable validation passed for service-principal")
}
...
func TestServicePrincipal_InputAssertions(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Input assertions passed for service-principal")
}
