...
func getTerraformDir(t *testing.T) string {
	...
	return filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main", "wrappers", "group-member"))
}

func getVarFile(t *testing.T) string {
	tfDir := getTerraformDir(t)
	return filepath.Join(tfDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-group-member.auto.tfvars")
}
...
func TestGroupMember_InitValidate(t *testing.T) {
	...
	t.Log("Init + Validate passed for group-member")
}
...
func TestGroupMember_Plan(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Plan passed for group-member")
}
...
func TestGroupMember_ApplyDestroy(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	group_id := terraform.Output(t, opts, "group_id")
	require.NotEmpty(t, group_id, "group_id should not be empty")
	member_id := terraform.Output(t, opts, "member_id")
	require.NotEmpty(t, member_id, "member_id should not be empty")

	t.Logf("id = %s", id)
	t.Logf("group_id = %s", group_id)
	t.Logf("member_id = %s", member_id)
}
...
func TestGroupMember_VariableValidation(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Variable validation passed for group-member")
}
...
func TestGroupMember_InputAssertions(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Input assertions passed for group-member")
}
