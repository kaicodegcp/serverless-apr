...
func getTerraformDir(t *testing.T) string {
	...
	return filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main", "wrappers", "user"))
}

func getVarFile(t *testing.T) string {
	tfDir := getTerraformDir(t)
	return filepath.Join(tfDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-user.auto.tfvars")
}
...
func TestUser_InitValidate(t *testing.T) {
	...
	t.Log("Init + Validate passed for user")
}
...
func TestUser_Plan(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
...
func TestUser_ApplyDestroy(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	user_name := terraform.Output(t, opts, "user_name")
	require.NotEmpty(t, user_name, "user_name should not be empty")
	...
	require.NotEmpty(t, home, "home should not be empty")

	t.Logf("id = %s", id)
	t.Logf("user_name = %s", user_name)
...
func TestUser_VariableValidation(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Variable validation passed for user")
}
...
func TestUser_InputAssertions(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Input assertions passed for user")
}
