...
func getTerraformDir(t *testing.T) string {
	...
	return filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main", "wrappers", "ip-access-list"))
}

func getVarFile(t *testing.T) string {
	tfDir := getTerraformDir(t)
	return filepath.Join(tfDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-ip-access-list.auto.tfvars")
}
...
func TestIpAccessList_InitValidate(t *testing.T) {
	...
	t.Log("Init + Validate passed for ip-access-list")
}
...
func TestIpAccessList_Plan(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Plan passed for ip-access-list")
}
...
func TestIpAccessList_ApplyDestroy(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	label := terraform.Output(t, opts, "label")
	require.NotEmpty(t, label, "label should not be empty")
	list_type := terraform.Output(t, opts, "list_type")
	require.NotEmpty(t, list_type, "list_type should not be empty")

	t.Logf("id = %s", id)
	t.Logf("label = %s", label)
	t.Logf("list_type = %s", list_type)
}
...
func TestIpAccessList_VariableValidation(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Variable validation passed for ip-access-list")
}
...
func TestIpAccessList_InputAssertions(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Input assertions passed for ip-access-list")
}
