...
func getTerraformDir(t *testing.T) string {
	...
	return filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main", "wrappers", "mws-credentials"))
}

func getVarFile(t *testing.T) string {
	tfDir := getTerraformDir(t)
	return filepath.Join(tfDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-mws-credentials.auto.tfvars")
}
...
func TestMwsCredentials_InitValidate(t *testing.T) {
	...
	t.Log("Init + Validate passed for mws-credentials")
}
...
func TestMwsCredentials_Plan(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Plan passed for mws-credentials")
}
...
func TestMwsCredentials_ApplyDestroy(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	credentials_id := terraform.Output(t, opts, "credentials_id")
	require.NotEmpty(t, credentials_id, "credentials_id should not be empty")
	credentials_name := terraform.Output(t, opts, "credentials_name")
	require.NotEmpty(t, credentials_name, "credentials_name should not be empty")
	external_id := terraform.Output(t, opts, "external_id")
	require.NotEmpty(t, external_id, "external_id should not be empty")

	t.Logf("id = %s", id)
	t.Logf("credentials_id = %s", credentials_id)
	t.Logf("credentials_name = %s", credentials_name)
...
func TestMwsCredentials_VariableValidation(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Variable validation passed for mws-credentials")
}
...
func TestMwsCredentials_InputAssertions(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Input assertions passed for mws-credentials")
}
