...
func getTerraformDir(t *testing.T) string {
	...
	return filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main", "wrappers", "mws-vpc-endpoint"))
}

func getVarFile(t *testing.T) string {
	tfDir := getTerraformDir(t)
	return filepath.Join(tfDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-mws-vpc-endpoint.auto.tfvars")
}
...
func TestMwsVpcEndpoint_InitValidate(t *testing.T) {
	...
	t.Log("Init + Validate passed for mws-vpc-endpoint")
}
...
func TestMwsVpcEndpoint_Plan(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Plan passed for mws-vpc-endpoint")
}
...
func TestMwsVpcEndpoint_ApplyDestroy(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	vpc_endpoint_id := terraform.Output(t, opts, "vpc_endpoint_id")
	require.NotEmpty(t, vpc_endpoint_id, "vpc_endpoint_id should not be empty")
	vpc_endpoint_name := terraform.Output(t, opts, "vpc_endpoint_name")
	require.NotEmpty(t, vpc_endpoint_name, "vpc_endpoint_name should not be empty")

	t.Logf("id = %s", id)
	t.Logf("vpc_endpoint_id = %s", vpc_endpoint_id)
	t.Logf("vpc_endpoint_name = %s", vpc_endpoint_name)
}
...
func TestMwsVpcEndpoint_VariableValidation(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Variable validation passed for mws-vpc-endpoint")
}
...
func TestMwsVpcEndpoint_InputAssertions(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Input assertions passed for mws-vpc-endpoint")
}
