...
func getTerraformDir(t *testing.T) string {
	...
	return filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main", "wrappers", "metastore"))
}

func getVarFile(t *testing.T) string {
	tfDir := getTerraformDir(t)
	return filepath.Join(tfDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-metastore.auto.tfvars")
}
...
func TestMetastore_InitValidate(t *testing.T) {
	...
	t.Log("Init + Validate passed for metastore")
}
...
func TestMetastore_Plan(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Plan passed for metastore")
}
...
func TestMetastore_ApplyDestroy(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	metastore_id := terraform.Output(t, opts, "metastore_id")
	require.NotEmpty(t, metastore_id, "metastore_id should not be empty")
	name := terraform.Output(t, opts, "name")
	require.NotEmpty(t, name, "name should not be empty")
	storage_root := terraform.Output(t, opts, "storage_root")
	require.NotEmpty(t, storage_root, "storage_root should not be empty")

	t.Logf("id = %s", id)
	t.Logf("metastore_id = %s", metastore_id)
	t.Logf("name = %s", name)
	t.Logf("storage_root = %s", storage_root)
}
...
func TestMetastore_VariableValidation(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Variable validation passed for metastore")
}
...
func TestMetastore_InputAssertions(t *testing.T) {
	terraformDir := getTerraformDir(t)
	varFile := getVarFile(t)
	...
	t.Log("Input assertions passed for metastore")
}
